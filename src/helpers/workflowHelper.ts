...
export function getColumnSolicitudExpediente(titles: string[]) {
  ...
  let currentCodExpedienteColumnName = [codigoSol, codigoSolAlt] ?? [
    "codigo solicitud",
  ];
...
export function getColumnCodigoExpediente(titles: string[]) {
  let currentCodExpedienteColumnName =
    import.meta.env.VITE_WF_COD_EXPE_COLS.toLowerCase().split("|") ?? [
      "expediente",
    ];

  return titles.findIndex((element) =>
    currentCodExpedienteColumnName.includes(element.toLowerCase())
  );
}

export function getColumnNameEstadoExpediente() {
  return (
    import.meta.env.VITE_WF_EST_EXPE_COLS.toLowerCase().split("|") ?? [
      "estado expediente",
    ]
  );
}
...
export function getColumnNameCanal() {
  return (
    import.meta.env.VITE_WF_CANAL_COLS.toLowerCase().split("|") ?? [
      "canal analisis solicitud",
    ]
  );
}
...
export function parseWfObjectByName(title: string, value: any): DatoExpediente {
  ...
  // float
  if (
    currentWfColumnFloat.includes(titleLower) ||
    titleLower.includes("importe")
  ) {
    let arrValue = parsedValue
      .split("")
      .filter((element: string) => [".", "."].includes(element) === false)
      .map((element: string) => (element === "," ? "." : element))
      .join("");
  ...
  if (
    titleLower.includes("fecha") ||
    currentWfColumnDate.includes(titleLower)
  ) {
    // parse to date
...
export function getNivel(expediente: Expediente) {
  ...
  let nivelColNames = [canal, canalAlt] || [
...
export function getImporteSolicitud(expediente: Expediente) {
  ...
  const importeSolicitudColNames = [importeOrigen, importeOrigenAlt] || [
    "Importe Solicitado",
  ];
  ...
  for (let monedaColName of monedaSolicitudColName) {
    let tryColName = ("" + expediente[monedaColName]).toLowerCase();

    if (tryColName) {
      if (
        tryColName.includes("usd") ||
        tryColName.includes("dolar") ||
        tryColName.includes("dólar")
      ) {
        found = "US$ " + found;
  ...
  if (found === "") {
    let aux: string =
      (expediente["Importe Solicitado"]
        ? (expediente["Importe Solicitado"] as number) / 1000
        : 0
      ).toFixed(0) + " M";
...
export function getLineaExpediente(expediente: Expediente) {
  let lineaExpedienteColNames =
    /* import.meta.env.VITE_WF_LINEA_EXPEDIENTE_COLS.split("|") ||  */ "SubLínea Solicitud";

  let found = (expediente[lineaExpedienteColNames] as string).toLowerCase();

  if (found.includes("amortizable")) {
    return found === "Amortizable Revolving".toLowerCase()
      ? "CPDP Rev"
      : "CPDP no rev";
  }
